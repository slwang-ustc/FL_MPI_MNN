...
import MNN
import sys
sys.path.append('/data/slwang/FL_MPI_MNN')
from config import cfg

F = MNN.expr


class AvazuDataset(MNN.data.Dataset):
    """
    ...
    def __init__(self, dataset_path=cfg['dataset_path'], cache_path=cfg['cache_path'], rebuild_cache=False, min_threshold=4, is_training=True):
        ...
        if self.is_training:
            self.labels = labels[:90000]
        else:
            self.labels = labels[90000:100000]
    ...
    def __getitem__(self, index):
        ...
        for i, feature_index in enumerate(np_array[1:]):
            embedding = np.zeros(self.field_dims[i])
            embedding[feature_index] = 1
            feature = np.concatenate((feature, embedding))
            
        feature = F.const(feature.flatten().tolist(), [int(np.sum(self.field_dims))], F.data_format.NCHW)
...
# dataset = AvazuDataset(dataset_path='/data/slwang/datasets/avazu/mini_set.csv', rebuild_cache=False, is_training=False)
# print(dataset.__getitem__(9999))
# print(len(dataset))
# print(dataset.__getitem__(1)[0][0].shape)
